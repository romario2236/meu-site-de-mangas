<template>
  <router-link :to="mangaDetailUrl" class="manga-link">
    <li class="manga-item">
      ...
      <button class="options-btn" @click.prevent="$emit('openMoveCopy', manga)">⋮</button>
    </li>
  </router-link>
</template>
...
// ALTERADO: Adicionado o novo evento 'openMoveCopy'
defineEmits(['removerManga', 'openMoveCopy']);
...
.manga-item img {
    width: 170px; height: 255px; border-radius: 8px;
...
.remover-btn {
    ...
    background-color: var(--remove-color); color: white;
...
/* NOVO: Estilos para o botão de opções */
.options-btn {
    position: absolute; bottom: 48px; right: 10px; /* Posicionado acima do nome */
    ...
    opacity: 0; transition: opacity 0.2s ease-in-out;
    z-index: 2;
    padding-bottom: 4px; /* Ajuste para centralizar os 3 pontos */
}

.manga-item:hover .options-btn {
    opacity: 1;
}
...
.chapter-overlay {
  ...
  z-index: 1; padding: 16px; box-sizing: border-box;
}
